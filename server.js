...
// 📩 Send the welcome DM AFTER Baileys is ready. Retries briefly and falls
// back to a plain-text message if the externalAdReply card fails.
async function sendWelcomeDm(sock, ownerJid, botName, sessionId) {
...
_Type .help in any chat to view your commands!_`;
  ...
  try {
    ...
    console.error("Welcome DM (card) failed, retrying as plain text:", cardErr.message);
  }

  // Fallback: plain text so the user still gets the message
  try {
    await sock.sendMessage(ownerJid, { text: welcomeDm });
    console.log(`📩 Welcome DM (plain) sent to ${ownerJid}`);
  } catch (plainErr) {
    console.error("Welcome DM (plain) also failed:", plainErr.message);
  }
}
...
async function startSession(sessionId, botName, cleanPhone) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Register FIRST so the bot shows up in the public directory + admin
      // even if the DM step is delayed. Only on a fresh pairing (cleanPhone set).
      if (cleanPhone) {
        try {
        ...
        // Sending in the same tick as 'open' is the classic reason it silently fails.
        setTimeout(() => {
          sendWelcomeDm(sock, ownerJid, botName, sessionId).catch(e =>
            console.error("sendWelcomeDm crashed:", e.message)
          );
        }, 5000);
      }
    }
