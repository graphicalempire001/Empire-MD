      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 24, scale: 0.9 }} transition={{ type: 'spring', stiffness: 320, damping: 26 }}
          ...
          >
            <div className="bg-gradient-green px-4 py-3 flex items-center gap-3">
              ...
              <button onClick={() => onOpenChange(false)} className="text-white/80 hover:text-white transition-colors" aria-label="Close chat">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-4 py-5 bg-white/50 min-h-[140px] flex flex-col gap-2">
              <div className="self-start max-w-[80%] bg-white rounded-2xl rounded-tl-sm px-3 py-2 shadow-sm">
                <p className="text-[13px] text-[#1a1a1a]">👋 Hi there! Type your message and we'll continue on WhatsApp.</p>
              </div>
            </div>
            ...
            <div className="p-2 bg-white/70 border-t border-black/[0.06] flex items-center gap-2">
              ...
              />
              <motion.button
                whileTap={{ scale: 0.9 }} onClick={send} disabled={!text.trim()}
