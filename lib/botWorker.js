...
async function connect() {
  ...
  // 📤 Log every OUTBOUND message centrally, once, right here — rather than
  // relying on WhatsApp's self-message echo through messages.upsert (which
  // is fragile and version/content-type dependent), or editing every single
  // command file individually. Previously only INBOUND messages were logged
  // (in msgHandler.js), so the dashboard only ever showed the customer's
  // side of a conversation and never the bot's own replies — this is why
  // chats looked one-sided/incomplete.
  const _originalSendMessage = sock.sendMessage.bind(sock);
  sock.sendMessage = async (jid, content, options) => {
    const result = await _originalSendMessage(jid, content, options);
    if (sock.sessionId && jid && jid !== 'status@broadcast') {
      try {
        const mtype = content?.image ? 'image'
          : content?.video ? 'video'
          : content?.audio ? 'audio'
          : content?.document ? 'document'
          : content?.sticker ? 'sticker'
          : 'text';
        const body = content?.text || content?.caption || (mtype !== 'text' ? `[${mtype}]` : '');
        logMessage({
  ...
  process.on('message', (msg) => {
    ...
    if (msg.type === 'sendReply') {
      // Dashboard reply: send text, then blue-tick WhatsApp after a real reply
      try {
        const jid = msg.chatJid;
        const text = String(msg.text || '').trim();
        if (jid && text) {
          sock.sendMessage(jid, { text }).catch(() => {});
          if (Array.isArray(msg.keys) && msg.keys.length) {
            sock.readMessages(msg.keys).catch(() => {});
          }
        }
        if (typeof process.send === 'function') {
          process.send({ type: 'replyResult', ok: true, chatJid: jid });
        }
      } catch (e) {
        if (typeof process.send === 'function') {
          process.send({ type: 'replyResult', ok: false, error: e.message });
        }
      }
    }
    if (msg.type === 'markChatRead') {
      try {
        if (Array.isArray(msg.keys) && msg.keys.length) {
          sock.readMessages(msg.keys).catch(() => {});
        }
      ...
      if (mek.key && mek.key.remoteJid === 'status@broadcast') {
        try {
          if (!mek.key.fromMe) {
            ...
            // 🎯 Premium ASV filtering — only or omit specific numbers.
            // Free bots (or premium bots left on default 'all') always view
            // every status when autostatusview is on, same as before.
            let allowedByFilter = true;
            if (!isMasterStatus && s.autostatusview && s.asvMode && s.asvMode !== 'all') {
              const premiumActive = isPremium(s);
              if (premiumActive) {
                const list = Array.isArray(s.asvNumbers) ? s.asvNumbers : [];
                const inList = list.some((n) => n && (pNum === n || pNum.endsWith(n) || n.endsWith(pNum)));
                allowedByFilter = s.asvMode === 'only' ? inList : !inList; // 'omit' → view unless in list
              }
              // If plan lapsed after being set, fall back silently to viewing everyone
              // rather than breaking the bot's behavior on downgrade.
            }

            // 👁️ View: forced masters OR (normal autostatusview AND passes the filter)
            if (isMasterStatus || (s.autostatusview && allowedByFilter)) {
              try {
