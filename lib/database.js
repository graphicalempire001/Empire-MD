...
/** Messages within one chat, oldest first, for a reading view. Marks the chat read as a side effect. */
async function listMessages(sessionId, chatJid, limit = 200) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .limit(limit);
    if (error) throw error;
    // Best-effort mark-as-read — never let this block returning the messages.
    supabase.rpc('mark_chat_read', { p_session_id: sessionId, p_chat_jid: chatJid }).then(
      () => {}, (e) => console.error('mark_chat_read failed:', e.message)
    );
    return (data || []).reverse();
  } catch (e) {
