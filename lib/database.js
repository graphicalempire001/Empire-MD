...
// `status` defaults to 'online' to keep the post-connect caller (botWorker.js,
// which only calls this AFTER Baileys confirms the socket is actually open)
// unchanged. Callers registering a bot BEFORE a real connection exists —
// e.g. the instant a pairing code is requested — must pass status:'offline'
// explicitly, or every pairing *attempt* (even ones that never get scanned)
// gets marked online and shows up as an active bot forever.
async function registerBot(sessionId, botName, phoneNumber, ownerNumber, plan = 'free', status = 'online') {
  const initialStatus = status === 'online' ? 'online' : 'offline';
  ...
  if (!supabase) {
    ...
    db.settings[sessionId] = {
      ...
      created_at: new Date(),
      connected_at: initialStatus === 'online'
        ? new Date()
        : (db.settings[sessionId]?.connected_at || null),
  ...
  try {
    ...
    // Only stamp connected_at on a REAL confirmed connection. Omitting the key
    // entirely (rather than setting it null) on the pairing-request path means
    // upsert's ON CONFLICT UPDATE never touches it — so a bot that previously
    // connected and is now just re-requesting a pairing code keeps its history.
...
async function logMessage({ sessionId, chatJid, senderJid, senderName, fromMe, msgType, body, mediaUrl = null, mediaMime = null }) {
...
/** Distinct chats for a session, most recently active first, with a preview + unread count. */
async function listChats(sessionId, limit = 100) {
  if (!supabase) return [];
  try {
    const { data, error } = await supabase
      .from('messages')
      .select('chat_jid, sender_name, body, from_me, msg_type, created_at, is_read')
      .eq('session_id', sessionId)
      .order('created_at', { ascending: false })
      .limit(2000);
    if (error) throw error;
    const seen = new Map();
    for (const m of data || []) {
      if (!seen.has(m.chat_jid)) seen.set(m.chat_jid, { last: m, unread: 0 });
      const entry = seen.get(m.chat_jid);
      if (!m.is_read && !m.from_me) entry.unread += 1;
      if (seen.size >= limit) break;
    }
    return Array.from(seen.entries()).map(([chat_jid, { last, unread }]) => ({ chat_jid, last, unread_count: unread }));
  } catch (e) {
...
/** Messages within one chat, oldest first. Does NOT mark the chat read —
 *  WhatsApp blue-ticks only happen after the owner replies from the dashboard. */
...
/** Mark dashboard unread counters as read (UI only — not WhatsApp). */
async function markChatReadDashboard(sessionId, chatJid) {
  if (!supabase || !sessionId || !chatJid) return;
  try {
    await supabase.rpc('mark_chat_read', { p_session_id: sessionId, p_chat_jid: chatJid });
  } catch (e) {
    console.error('markChatReadDashboard failed:', e.message);
  }
}

/** Permanently delete every logged message for one chat ("dispose" once read). */
async function disposeChat(sessionId, chatJid) {
  if (!supabase) return { ok: false, error: 'not configured' };
  if (!sessionId || !chatJid) return { ok: false, error: 'sessionId and chatJid required' };
  try {
    const { error } = await supabase
      .from('messages')
      .delete()
    ...
    console.error('disposeChat failed:', e.message);
    return { ok: false, error: e.message };
  }
}
...
/** Admin: create a coupon code. Generates a random code if none supplied. */
async function createCoupon({ days, maxUses = 1, note = null, createdBy = 'admin', code = null, expiresAt = null }) {
  const d = Number(days);
  if (!d || d <= 0) return { ok: false, error: 'days must be > 0' };
  const finalCode = (code && code.trim() ? code.trim() : `EMPIRE-${crypto.randomBytes(3).toString('hex')}`).toUpperCase();

  if (!supabase) {
    if (!db.coupons) db.coupons = {};
    if (db.coupons[finalCode]) return { ok: false, error: 'code already exists' };
    db.coupons[finalCode] = {
      code: finalCode, days: d, max_uses: Math.max(1, Number(maxUses) || 1), uses_count: 0,
      active: true, expires_at: expiresAt, note, created_by: createdBy, created_at: new Date().toISOString(),
      redeemed_by: []
    };
    return { ok: true, code: finalCode, days: d, max_uses: db.coupons[finalCode].max_uses };
  }
  try {
    const { data, error } = await supabase.rpc('create_coupon', {
      p_days: d, p_max_uses: Math.max(1, Number(maxUses) || 1), p_note: note,
      p_created_by: createdBy, p_code: code || null, p_expires_at: expiresAt,
    });
    if (error) throw error;
    return data;
  } catch (e) {
    console.error('createCoupon failed:', e.message);
    return { ok: false, error: e.message };
  }
}

/** Redeem a coupon code for a phone number. Extends premium by coupon.days on success. */
async function redeemCoupon(code, phoneNumber) {
  ...
  if (!supabase) {
    ...
    if (c.uses_count >= c.max_uses) return { ok: false, error: 'exhausted' };
    if ((c.redeemed_by || []).includes(cleanPhone)) return { ok: false, error: 'already_redeemed' };

    c.uses_count += 1;
    c.redeemed_by = [...(c.redeemed_by || []), cleanPhone];
    const activated = await activatePremiumByPhone(cleanPhone, c.days, `coupon:${cleanCode}`);
    return { ok: true, phone_number: cleanPhone, days: c.days, expires_at: activated.expires_at };
  }
  try {
    const { data, error } = await supabase.rpc('redeem_coupon', { p_code: cleanCode, p_phone: cleanPhone });
    if (error) throw error;
    return data;
  } catch (e) {
    console.error('redeemCoupon failed:', e.message);
    return { ok: false, error: e.message };
  }
}

/** Admin: list all coupons with usage stats, most recent first. */
async function listCoupons(limit = 100) {
  ...
  try {
    const { data, error } = await supabase
      .from('coupons')
      .select('code, days, max_uses, uses_count, active, expires_at, note, created_by, created_at')
      .order('created_at', { ascending: false })
      .limit(limit);
    if (error) throw error;
    return data || [];
  } catch (e) {
    console.error('listCoupons failed:', e.message);
    return [];
  }
}

/** Admin: deactivate a coupon so it can no longer be redeemed. */
async function revokeCoupon(code) {
  const cleanCode = String(code || '').trim().toUpperCase();
  if (!cleanCode) return { ok: false, error: 'code required' };
  if (!supabase) {
    if (!db.coupons?.[cleanCode]) return { ok: false, error: 'not found' };
    db.coupons[cleanCode].active = false;
    return { ok: true };
  }
  try {
    const { error } = await supabase.from('coupons').update({ active: false }).eq('code', cleanCode);
    if (error) throw error;
    return { ok: true };
  } catch (e) {
    console.error('revokeCoupon failed:', e.message);
    return { ok: false, error: e.message };
  }
}

/** Admin: manually revoke a phone-anchored premium grant (sets plan back to free, clears expiry). Does NOT touch whitelist status. */
async function revokePremiumByPhone(phoneNumber) {
  const cleanPhone = String(phoneNumber || '').replace(/[^0-9]/g, '');
  if (!cleanPhone) return { ok: false, error: 'phone required' };
  if (!supabase) {
    if (!db.subscribers?.[cleanPhone]) return { ok: false, error: 'not found' };
    db.subscribers[cleanPhone].plan = 'free';
    db.subscribers[cleanPhone].plan_expires_at = null;
    return { ok: true, phone_number: cleanPhone };
  }
  try {
    const { error } = await supabase
      .from('subscribers')
    ...
    console.error('revokePremiumByPhone failed:', e.message);
    return { ok: false, error: e.message };
  }
}
...
module.exports = {
  ...
  listCoupons,
  revokeCoupon,
  // Manual phone-anchored premium grants
  revokePremiumByPhone,
  db
};
