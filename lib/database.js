...
async function registerBot(sessionId, botName, phoneNumber) {
    ...
    const defaultSettings = {
        ...
        alwaysOnline: true,
        welcome: true,
        ownerNumber: cleanPhone ? [cleanPhone] : []   // ← Force owner here
    };
    ...
    if (!supabase) {
        ...
        console.log(`[DB] In-memory owner set for ${sessionId}: ${cleanPhone}`);
        return;
    }
    ...
    try {
        ...
        const mergedSettings = {
            ...defaultSettings,
            ...existing,
            ownerNumber: Array.isArray(existing.ownerNumber) && existing.ownerNumber.length > 0 
                ? existing.ownerNumber 
        ...
        else console.log(`[DB] Owner registered for ${sessionId}: ${cleanPhone}`);
    } catch (e) {
