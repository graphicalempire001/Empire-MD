// Empire MD - Core Message Handler (Per-Bot Owner, Final + fromMe fix + LID resolve + Group Body fix + Abuse Gate + Antilink + Greet + Away + Presence)
const config = require('../config');
const { getSettings, isBotAbusive, db, updateSettings } = require('./database');
const commands = require('./commands');

// Message keys that are NOT real content — must be skipped (present in group messages)
const IGNORE_KEYS = new Set([
    'senderKeyDistributionMessage',
    'messageContextInfo'
]);

// Whitelist of commands that ALWAYS bypass the private mode gate
...
// Resolve a group participant to its real phone-number JID.
// Modern Baileys reports group senders as "xxxx@lid"; the owner check needs the real number.
async function resolveRealJid(sock, chatJid, isGroup, mek, rawSender) {
    if (mek.key.participantPn) return mek.key.participantPn;      // best case
    if (!isGroup || !rawSender.endsWith('@lid')) return rawSender;

    try {
        // Try Baileys' own LID→PN mapping store first (newer versions)
...
const LINK_REGEX = /(chat\.whatsapp\.com/[A-Za-z0-9]+|wa\.me/|t\.me/|https?://|www\.[^\s]+|[a-z0-9-]+\.(com|net|org|io|xyz|me|link|gg|info|biz))/i;
...
// ─── Apply per-bot presence (typing / recording / always-online) ───
...
async function handleMessage(sock, mek) {
    try {
        ...
        // ✅ Pick the REAL content key — skip group envelope keys (senderKeyDistributionMessage, etc.)
        ...
        // ✅ Normalize @lid group senders to the real phone-number JID
        const sender = fromMe
            ? botId
        ...
        // ✅ Body extraction — msg is already the inner node/string
        let body =
        ...
        // ⚙️ APPLY AUTO PRESENCE (typing / recording / always-online) per-bot
        ...
        console.log(`[OWNER DEBUG] Session:${sock.sessionId || 'N/A'} | RawSender:${rawSender} | Sender:${cleanSender} | Bot:${connectedNumber} | Owners:${JSON.stringify(ownerNumbers)} | fromMe:${fromMe} | IsOwner:${isOwner} | Mode:${currentMode} | Body:"${body}"`);
...
*Since:* ${new Date(afkData.time).toLocaleTimeString()}`,
        ...
        // Private mode protection - except for whitelisted public commands
        if (currentMode === "private" && !isOwner && !PUBLIC_ALWAYS.has(command)) {
...
Type *${prefix}help* for the available commands.` 
            }, { quoted: mek });
        }
    } catch (err) {
