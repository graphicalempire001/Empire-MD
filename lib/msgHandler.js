// Empire MD - Core Message Handler (Per-Bot Owner, Final)
...
async function handleMessage(sock, mek) {
    try {
        ...
        // === DEBUG (remove later) ===
        const cleanSender = sender.replace(/[^0-9]/g, '');
        console.log(`[OWNER DEBUG] Session:${sock.sessionId || 'N/A'} | Sender:${cleanSender} | Bot:${connectedNumber} | Owners:${JSON.stringify(ownerNumbers)} | IsOwner:${isOwner} | Mode:${currentMode}`);
