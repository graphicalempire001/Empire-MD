// Empire MD - Core Message Handler (Per-Bot Owner, Final + fromMe fix + LID resolve + Abuse Gate + Antilink + Greet + Away + Presence)
...
async function resolveRealJid(sock, chatJid, isGroup, mek, rawSender) {
    // Fast path: some Baileys versions attach the phone-number form directly.
    if (mek.key.participantPn) return mek.key.participantPn;
    if (!isGroup || !rawSender.endsWith('@lid')) return rawSender;

    try {
        const meta = await sock.groupMetadata(chatJid);
        const match = meta.participants.find(p => p.id === rawSender || p.lid === rawSender);
        if (match) return match.jid || match.id || rawSender;
    } catch (e) {
