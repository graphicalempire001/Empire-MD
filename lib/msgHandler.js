...
const {
  isPremium, isPremiumCommand, freePromoFooter, premiumRequiredMsg
} = require('./premium');
...
*Since:* ${new Date(afkData.time).toLocaleTimeString()}`,
    ...
    if (commands[command]) {
      try {
        ...
        // Free-tier promo footer
        if (!botIsPremium && !ghost && !['upgrade', 'plan', 'help', 'h', 'menu', 'list'].includes(command)) {
          try {
            await sock.sendMessage(chatJid, {
              text: freePromoFooter(prefix)
            });
          } catch (_) {}
        }
