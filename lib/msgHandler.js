...
async function handleMessage(sock, mek) {
    try {
        ...
        // Convenient shortcut for moderation commands
...
*Since:* ${new Date(afkData.time).toLocaleTimeString()}`,
        ...
        // Run command — now every command receives the quoted context
        if (commands[command]) {
            await commands[command]({
                ...
                prefix,
                settings,
                quoted: mek.quoted,             // <-- added
                quotedSender: mek.quotedSender, // <-- added
                contextInfo                     // <-- added (handy for mentionedJid)
            });
        } else {
