// Empire MD - Core Message Handler (Final Fixed Version)
const config = require('../config');
const { getSettings, db } = require('./database');
const commands = require('./commands');

// Highly flexible owner checker
function isOwnerCheck(sender, botId, customOwners = []) {
    ...
    return owners.some(owner => {
        ...
        return cleanSender === cleanOwner ||
               ...
               cleanSender.slice(-8) === cleanOwner.slice(-8); // last 8 digits
    });
}
...
async function handleMessage(sock, mek) {
    try {
        ...
        // === DETAILED DEBUG (remove later) ===
        const cleanSender = sender.replace(/[^0-9]/g, '');
        console.log(`[OWNER DEBUG] Session:${sock.sessionId || 'N/A'} | Sender:${cleanSender} | Bot:${botId.replace(/[^0-9]/g,'')} | DB Owners:${JSON.stringify(ownerNumbers)} | IsOwner:${isOwner} | Mode:${currentMode}`);
