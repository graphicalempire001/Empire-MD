...
async function handleMessage(sock, mek) {
  try {
    ...
    if (!isCmd) {
      ...
      // === 🤖 AI CONVERSATION MODES (mention / swipe-reply / aggressive + self-owner) ===
      const aiMode = settings.aichatmode || "off";
      if (aiMode !== "off" && body && body.trim()) {
        const cleanBot = connectedNumber.replace(/[^0-9]/g, '');

        // Loop guard: never let the bot answer its own AI/status output.
        ...
        const shouldAnswer = !isSelfAiEcho && (
          (!fromMe && (repliedToBot || mentionedBot)) ||          // any user: reply or mention
          (!fromMe && aiMode === "aggressive" && !isGroup) ||     // aggressive: all DMs
          (fromMe && (repliedToBot || mentionedBot))              // OWNER on bot's own number: reply/mention
        );
