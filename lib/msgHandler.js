// Empire MD - Core Message Handler (Per-Bot Owner + AI Conversation Engine: mention / swipe / aggressive)
...
async function handleMessage(sock, mek) {
  try {
    ...
    if (!isCmd) {
      ...
      // === 🤖 AI CONVERSATION MODES (mention / swipe-reply / aggressive) ===
      const aiMode = settings.aichatmode || "off";
      if (aiMode !== "off" && !fromMe && body && body.trim()) {
        const cleanBot = connectedNumber.replace(/[^0-9]/g, '');

        // Was the bot swipe-replied to?
        const repliedToBot = mek.quotedSender
          ? mek.quotedSender.replace(/[^0-9]/g, '').endsWith(cleanBot)
          : false;
        ...
        const shouldAnswer =
          repliedToBot ||                                // reply → answer (any active mode)
          mentionedBot ||                                // mention → answer (any active mode)
          (aiMode === "aggressive" && !isGroup);         // aggressive → answer all DMs

        if (shouldAnswer) {
          // Strip the @mention token from the text so the model sees a clean prompt
          let aiText = body;
          if (mentionedBot) {
            aiText = aiText.replace(new RegExp(`@${cleanBot}`, 'g'), '').trim();
          }
          if (aiText) {
            try {
              ...
              console.error("AI auto-trigger failed:", e.message);
            }
            return; // handled by AI; skip greet/away fallthrough
          }
        }
      }
