...
// NOTE: jimp v1.x API (named export, no static Jimp.MIME_* constants,
// resize takes {w,h}). Pinned to whatever baileys' peerDependency wants
// — do NOT force jimp@0.22.x, it conflicts with @whiskeysockets/baileys.
...
async function fetchThumb(url) {
  ...
  try {
    ...
    console.error(`channelThumb fetch failed (${status}) for ${url}: ${e.message}`);
    return null;
  }
}
...
async function stampVerifiedBadge(thumbBuffer) {
  ...
  try {
    ...
    const targetSize = Math.round(Math.min(base.width, base.height) * 0.32);
    badge.resize({ w: targetSize, h: targetSize });

    const x = base.width - targetSize - Math.round(targetSize * 0.08);
    const y = base.height - targetSize - Math.round(targetSize * 0.08);
